"""VoiceGuard V2 — Phase 7 replay-detector training scaffold.

Synthetic-only validation.  Real replay-detection training requires
ASVspoof 2019/2021 datasets; the training pipeline is compatible with
that data once it becomes available.  No production AUC-ROC, ACER,
BPCER, or APCER is claimed from synthetic runs.
...
class SyntheticReplayDataset(Dataset[tuple[Tensor, Tensor]]):
    """Deterministic synthetic live/replay mel-spectrogram pairs.

    Each sample is a random mel-spectrogram with a binary label:
    - label 0 (live): random spectral template + natural variation
    - label 1 (replay): template with channel coloration + quantization
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(n_samples):
            ...
            mel = template + noise
            if label == 1:
                # Simulate replay: apply channel coloration (flatten spectrum)
                coloration = rng.uniform(0.8, 1.2, (n_mels, 1)).astype(np.float32)
                mel = mel * coloration
                # Add quantization-like clipping.
                mel = np.clip(mel, -2.0, 2.0)
...
class ReplayTrainingConfig:
    """Configuration for replay-detector smoke training."""
...
def run_training(
    ...
    *,
    ...
    """Train the replay detector on synthetic data (1-epoch smoke).

    Returns epoch-averaged losses.  The resulting model is NOT a useful
    replay detector — this only validates that the training machinery runs.
    """
    cfg = config or ReplayTrainingConfig()
    set_seed(cfg.seed)
    dev = device or torch.device("cpu")
    ...
    dataset = SyntheticReplayDataset(
