"""Tests for the Phase 7 replay-detector model (1-D CNN)."""
...
from voiceguard.ml.models.anti_spoofing_replay import ReplayDetector, pool_length
...
class TestReplayDetector:
    ...
    def test_rejects_wrong_n_mels(self) -> None:
        model = ReplayDetector(n_mels=80)
    ...
    def test_predict_returns_is_replay(self) -> None:
        model = ReplayDetector()
        mel = torch.randn(2, 1, 80, 100)
        logits, probs, is_replay = model.predict(mel, threshold=0.5)
        assert logits.shape == (2, 1)
        assert probs.shape == (2, 1)
        assert is_replay.shape == (2, 1)
        assert is_replay.dtype == torch.bool
        assert bool((is_replay == (probs >= 0.5)).all())

    def test_custom_cnn_channels(self) -> None:
        model = ReplayDetector(cnn_channels=(16, 32, 64))
    ...
    def test_rejects_wrong_cnn_channels(self) -> None:
        with pytest.raises(AntiSpoofError, match="cnn_channels"):
            ReplayDetector(cnn_channels=(32, 64))
